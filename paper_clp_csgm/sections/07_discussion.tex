...
The ablation results strengthen this interpretation. The prior-only decoder is
weaker than the full method, which shows that sparse measurements add useful
target-well information beyond the log-conditioned latent estimate. The
measurement-only CSGM variant is also weaker, especially at low \(\rho\), which
shows that the sparse observations alone do not sufficiently identify a stable
latent code. The method works best when both sources are used together: logs
anchor the latent search, and sparse measurements correct it.

The Ridge-versus-MLP sensitivity study further supports the low-data design
choice. A more flexible MLP prior does not improve the conditional latent prior
in the tested grids; it is consistently worse than Ridge. This suggests that,
with few training windows, regularized linear latent prediction is preferable to
a higher-capacity prior that can overfit or become unstable.
...
core measurements should be treated as measurement evidence in the target well.
