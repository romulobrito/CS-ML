...
e o cenário NMR VHR (perfil $K_f(z)$).
...
de Voigt--Reuss--Hill (VHR). Não é correção física da mistura iso-tensão
...
$\approx 0{,}31$ ($S_o\approx 0{,}69$), $K_f$ VHR $\approx 1{,}40$~GPa
...
\begin{figure}[H]
  ...
  \caption{Alternativa NMR com mistura VHR: $S_w$/\texttt{SWIRR}
  (recortes em vermelho), $K_f$ VHR e $C_f=1/K_f$.
  Verde: $K_f=2{,}2$~GPa adotado; laranja: mediana VHR
  ($\approx 1{,}40$~GPa).}
  \label{fig:kf_sw_nmr}
\end{figure}
...
\begin{table}[H]
  ...
  só o fluido no Gassmann muda. NMR = mistura VHR no perfil $K_f(z)$.}
...
VHR piora um pouco o MAPE frente ao adotado (Gassmann
$\approx +0{,}35$~p.p.; RF $\approx +0{,}14$~p.p.); a linear fica igual.
Perfil $K_f(z)$ e mediana VHR produzem métricas \emph{próximas, não iguais}
(7,63\% vs 7,62\%).
Isso não demonstra que $K_f$ varie pouco
(VHR $\approx 1{,}0$--$2{,}1$~GPa neste intervalo):
...
poço e VHR+\texttt{SWIRR} ficam como testes de sensibilidade.
...
($\approx 2{,}83$~GPa) e VHR+\texttt{SWIRR} (figura NMR).
...
VHR+\texttt{SWIRR} não melhoram o produto neste intervalo).
...
\begin{enumerate}
  ...
  \item \textbf{Fluido:} $K_f=2{,}2$~GPa ($S_w=1$) permanece no produto;
        mediana de poço ($\approx 2{,}83$~GPa) e VHR+\texttt{SWIRR}
...
A sensibilidade a fluido ($K_f$, $S_w$, VHR+\texttt{SWIRR}) é regenerada por
