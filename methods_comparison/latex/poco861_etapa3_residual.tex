...
\begin{table}[H]
  ...
  \begin{tabular}{p{2.2cm}p{3.8cm}p{4.2cm}p{3.8cm}}
    \toprule
    Etapa & Resultado principal & Entrada-chave & Papel na Etapa~3 \\
    \midrule
...
aceita os resultados validados e pergunta apenas se o que \emph{sobra} frente ao
...
A Tabela~\ref{tab:hybrid_def} fixa a aritmética do modelo híbrido conforme
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Modelo avaliado & $V_{\mathrm{p,hybrid}} = V_{\mathrm{p,Gassmann}} + \widehat{\Delta V_{\mathrm{p}}}$ \\
...
Antes de apresentar o modelo híbrido, a Figura~\ref{fig:gassmann_depth_ref}
...
\begin{figure}[H]
  ...
  \caption{MAPE de \Vp{} híbrida OOF versus DSI por regressor. Linha tracejada:
           ...
           baseline; RF (barra vermelha) permanece o modelo adotado e a regressão
...
A Figura~\ref{fig:residual_scatter} exibe o ajuste OOF do RF (modelo adotado)
...
Figura~\ref{fig:gassmann_depth_ref}) e \Vp{} híbrida OOF (modelo desta etapa).
...
casamento DSI) antes de exportar \Vp{} híbrida para o perfil teórico.
...
\textbf{O que a estratificação informa.}
...
\textbf{O que o sweep informa na prática.}
\begin{itemize}
...
Na prática, o que limita não é o algoritmo, e sim quantas profundidades
têm sónico no treino.
...
O modelo adotado mantém fluido \textbf{constante} no Gassmann:
...
o resultado (MAPE/viés) é pouco sensível a usar o perfil ou a mediana.
...
\textbf{(iii)} Fluido no Gassmann: o modelo adotado usa PVT fixo
...
Nenhuma melhorou o acordo frente ao sônico neste intervalo, mas pressão efetiva e
...
Mantemos o RF no modelo documentado por alinhamento à Etapa~1; testes futuros
...
a coluna $V_{\mathrm{p,hybrid}}$ (RF ou regressão linear) ao perfil
...
VRH+\texttt{SWIRR} não melhoram o acordo neste intervalo).
...
\begin{enumerate}
  ...
  \item \textbf{Fluido:} $K_f=2{,}2$~GPa ($S_w=1$) permanece no modelo adotado;
