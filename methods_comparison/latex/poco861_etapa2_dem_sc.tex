...
monoescala Gassmann, confirmando manter o modelo monoescala.
...
\subsection{Fontes e resultados}
...
testa o modelo final ao longo do poço~861 em condições distintas do
laboratório seco.
...
\subsection{O que MAPE, RMSE e viés informam}
...
apenas de verificação e \textbf{não} é usado no modelo adotado.
...
permanecem documentadas para uso futuro, mas nao entram no modelo atual.
...
menor ou nenhum suporte experimental; a HFU~4 não deve entrar no modelo
...
A Tabela~\ref{tab:validation_summary} mostra dois regimes de erro.
Nos plugs, o LOO ($\approx 15\%$) tem viés praticamente nulo: o modelo
não está alto nem baixo de forma sistemática; o que resta é dispersão
entre dez pontos a sustentar três conjuntos de parâmetros.
...
A Etapa~3 (relatório residual) reavaliou o default $K_f=2{,}2$~GPa frente à
mediana de \texttt{KFluid} no intervalo ($\approx 2{,}83$~GPa) e à mistura
NMR VRH com \texttt{SWIRR} (figura da Etapa~3).
Nenhum desses cenários melhorou o acordo com o sónico nem o híbrido OOF
neste poço; o modelo adotado mantém $K_f=2{,}2$~GPa e $S_w=1$.
...
\textbf{Conclusão:} manter o modelo monoescala Gassmann no perfil de 87
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[arr] (q) -- (n);
  \end{tikzpicture}
  \caption{Critério de decisão das fases~2f e~2g: o modelo de 87 linhas
...
Em termos práticos, o modelo desta etapa é mais confiável para
...
Mantemos $\alpha$ efetivo por HFU nas 87 linhas; as fracoes
...
justificou trocar o monoescala por inclusoes sequenciais no perfil teórico
