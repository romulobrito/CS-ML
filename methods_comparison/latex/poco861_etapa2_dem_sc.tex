...
a seco é $\approx 16\%$, com viés $+0{,}62$~km/s.
A fase~2b aplica Gassmann com fluido de formação ($K_f \approx 2{,}2$~GPa,
saturação total) e reduz parcialmente o viés para $+0{,}59$~km/s
(MAPE $\approx 15\%$), sem fechar completamente o gap frente ao sónico;
\VpVs{} concorda melhor a seco (erro médio absoluto $\approx 0{,}07$) do que
após Gassmann ($\approx 0{,}07$--0,08).
A fase~2g testa DEM multiescala sequencial com frações macro/micro do CMR
ponto a ponto no perfil de 87 linhas; o ganho MAPE frente ao monoescala
Gassmann é de apenas $\approx 1{,}3$~p.p. (abaixo do limiar pre-registrado de
2~p.p.), confirmando manter o produto monoescala.
A HFU~2 apresenta o melhor acordo ($\approx 8\%$); a HFU~4, sem plugs de
calibração, diverge ($\approx 37\%$).
...
contribui.
A Seção~\ref{sec:protocolo} descreve a cadeia física e os protocolos de
validação.
...
Seção~\ref{sec:multiscale_nmr} testa frações NMR/CMR no perfil completo.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.2cm}p{4.8cm}rc}
    ...
    Perfil sónico DSI (DLIS)
      & DTCO, DTSM, \VpVs{} de perfil & 190 no poço~861 & Contínuo \\
    CMR/ECS (DLIS)
...
Nesta etapa ainda não aplicamos correcção de saturação por
Gassmann; $K_{\mathrm{dry}}$ e $G_{\mathrm{dry}}$ referem-se à rocha a seco.
...
\subsection{Entradas por escala}
...
\begin{itemize}
  \item $\alpha$ (razão de aspecto), limitada ao intervalo $[0{,}05,\,0{,}95]$;
...
laboratório.
Quando $\alpha$ calibrado cola no limite superior (0,95) ou inferior (0,19),
isso indica que a mediana do microCT, sozinha, não descreve a elástica
daquela unidade de fluxo.
...
Um modelo pode ter MAPE moderado e viés alto se errar sempre na mesma
direção---foi o que ocorreu no confronto com o sónico de perfil.
...
profundidade do registro para metros (escala 408,1 ancorada nas profundidades
do perfil integrado) e obtemos \Vp{} pela relação
\begin{equation}
  \label{eq:vp_dlis}
...
sónicos de poço.
Cada uma das 87 linhas do perfil DEM foi casada com a amostra sónica mais
próxima (tolerância 0,25~m).
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \Vp{} médio DEM & 6,26 & km/s \\
    \VpVs{} médio DEM & 1,84 & -- \\
    Diferença relativa média DEM vs SC (\Vp) & 0,37 & \% \\
    \bottomrule
  \end{tabular}
\end{table}

A Tabela~\ref{tab:poc} confirma que o valor médio de \Vp{} $\approx 6{,}26$~km/s é alto para carbonatos do campo Lapa
...
75\%); $G_{\mathrm{dry}}$ diverge em média 8\% (máximo 17\%); \Vp{} a seco
diverge em média 7\% (máximo 15\%).
...
\begin{table}[H]
  ...
  \begin{tabular}{ccccccccc}
    ...
    Escala $K,G$ & Origem \\
    \midrule
    1 & 4 & 0,950 & 0,554 & 58,6 & 25,8 & 2,774 & 0,698 & laboratório \\
    2 & 4 & 0,191 & 0,554 & 58,7 & 25,6 & 2,766 & 0,706 & laboratório \\
    3 & 2 & 0,950 & 0,615 & 59,0 & 25,5 & 2,754 & 0,721 & laboratório \\
    4 & 0 & 0,570 & -- & 82,5 & 35,8 & 2,760 & 1,000 & \emph{fallback} \\
    \bottomrule
  \end{tabular}
\end{table}
...
imagem CT.
A HFU~2 é a mais bem sustentada (quatro plugs, $\alpha$ calibrado 0,19).
...
\emph{in-sample} da HFU~2 caindo para $\approx 7{,}5\%$; a rodada de produção
documentada aqui usa a configuração padrão com dez plugs, salvo análise
de sensibilidade explícita.
...
mais se aproximam ($\alpha$ calibrado 0,19 versus mediana CT $\approx 0{,}55$,
...
Este patamar ($\approx 15\%$, RMSE $\approx 0{,}85$~km/s) servirá de
referência para julgar o confronto com o sónico de perfil
(Seção~\ref{sec:dlis}).
...
permanece $\approx 9{,}1\%$, praticamente identico ao M1
($\Delta \approx 0{,}01$~p.p.).
No LOO \emph{forward}, o MAPE cai de $\approx 14{,}8\%$ (M1) para
$\approx 12{,}3\%$ (M2b)---ganho de $\approx 2{,}5$~p.p., porém
...
\begin{table}[H]
  ...
  \begin{tabular}{llcc}
    ...
    M2a multiescala \emph{oracle} & In-sample & 3,0 & 0,38 \\
    M2b multiescala \emph{forward} & In-sample & 9,1 & 0,55 \\
    M1 monoescala calibrado & LOO & 14,8 & 0,85 \\
    M2b multiescala \emph{forward} & LOO & 12,3 & 0,76 \\
    \bottomrule
  \end{tabular}
\end{table}
...
da Tabela~\ref{tab:validation_summary}, o MAPE de \Vp{} ($\approx 16\%$)
aproxima-se do LOO laboratorial (Tabela~\ref{tab:loo}), mas o viés é
positivo ($+0{,}62$~km/s): o DEM calibrado a seco fica mais rápido que o
sónico \emph{in-situ}.
\VpVs{} apresenta acordo melhor na mesma tabela (erro médio absoluto
$\approx 0{,}07$; médias 1,78 versus 1,82).
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    MAPE \Vp & 16,2 & \% \\
    RMSE \Vp & 0,85 & km/s \\
    Viés \Vp (DEM $-$ sónico) & $+0{,}62$ & km/s \\
    \Vp{} médio sónico & 4,58 & km/s \\
    \Vp{} médio DEM & 5,20 & km/s \\
...
Os numeros da Tabela~\ref{tab:dlis_global} mostram que o MAPE de $\approx 16\%$ \emph{sozinho} poderia sugerir modelo aceitável---
mas o viés de $+0{,}62$~km/s revela que o erro não é simétrico: o DEM
superestima \Vp{} de forma consistente frente ao sónico saturado.
Isso é fisicamente plausível: calibramos em plugs secos a 22,1~MPa e
aplicamos ao perfil sem correção de fluido (Gassmann).
O RMSE igual ao LOO ($\approx 0{,}85$~km/s) indica que a ordem de grandeza do
erro se mantém entre laboratório e perfil, embora por mecanismos distintos
(viés baixo no LOO, viés alto no sónico).
...
Na Tabela~\ref{tab:dlis_hfu}, a HFU~2 ($\approx 8\%$) é a única com erro próximo ao patamar desejável
para interpretação quantitativa---coerente com quatro plugs de
calibração e viés baixo.
A HFU~1 cobre quase metade do perfil (42 linhas) com MAPE $\approx 20\%$:
utilizável com ressalvas.
A HFU~4 ($\approx 37\%$) confirma que extrapolar sem laboratório é arriscado;
não deve entrar em produto final sem novos dados.
...
O sónico (verde) apresenta mais variabilidade fina que o DEM (azul), que
tende a suavizar tendências locais após calibração por HFU.
Ainda assim, as duas curvas permanecem na mesma faixa de km/s na maior parte do
intervalo.

\emph{Painel (b)---dispersograma:}
A HFU~2 concentra-se próxima da diagonal; HFU~4 aparece sistematicamente
acima da linha 1:1 (superestimação).
...
Na Figura~\ref{fig:dlis_vpvs}, o painel de \VpVs{} mostra trilhas próximas---confirmando que a razão
elástica é melhor capturada que o módulo absoluto.
O mapa de erro versus profundidade evidencia faixas onde a HFU~1 e a HFU~4
concentram os maiores desvios, orientando onde a fase~2b (saturação) ou
novos dados de laboratório teriam maior impacto.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    Perfil sónico DSI (seco) & 16,2 & 0,85 & $+0{,}62$ & 87 \\
    Perfil sónico DSI (Gassmann) & 15,4 & 0,81 & $+0{,}59$ & 87 \\
    Perfil DSI (Gassmann + NMR M3) & 14,0 & 0,74 & $+0{,}30$ & 87 \\
    \bottomrule
  \end{tabular}
\end{table}

A leitura conjunta da Tabela~\ref{tab:validation_summary} revela que o MAPE permanece na faixa 14--16\% nas validações honestas (LOO e
sónico), mas o \textbf{viés muda de sinal}: quase nulo no laboratório
seco após calibração, positivo no perfil saturado.
A linha Gassmann mostra redução modesta do viés ($+0{,}62 \rightarrow +0{,}59$~km/s);
a linha M3 (NMR multiescala) reduz ainda mais o viés ($+0{,}30$~km/s), porém
sem atingir o limiar de ganho MAPE pre-registrado (Seção~\ref{sec:multiscale_nmr}).
Confirmando que a diferença seca/saturada explica \emph{parte}, mas não a
totalidade, do desvio contra o DSI (Seção~\ref{sec:gassmann}).
...
observado: o modelo permanece $\approx 0{,}6$~km/s mais rápido que o DSI em
média.
...
\begin{table}[H]
  ...
  \begin{tabular}{llcc}
    ...
    Saída DSI (seco) & Extrapola \Vp{} \emph{in-situ}? & Viés (km/s) & $+0{,}62$ \\
    Saída DSI (Gassmann) & Fluido reduz o viés? & Viés (km/s) & $+0{,}59$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
mas a \emph{saída} contra o DSI permanece com viés positivo mesmo após
Gassmann---motivando a Etapa~3 sobre resíduos, não o abandono do DEM/SC.
...
não melhorou o MAPE: 15,4\% versus 15,4\%).
...
O M3 reduz o MAPE de 15,4\% (M1) para 14,0\% e o viés de $+0{,}59$ para
$+0{,}30$~km/s, mas o ganho $\Delta \approx 1{,}3$~p.p. fica \textbf{abaixo}
do limiar pre-registrado de 2~p.p.
A correlação de Pearson entre \Vp{} predito e sónico permanece negativa e
próxima de zero em ambos os modelos ($r \approx -0{,}17$ e $-0{,}13$),
indicando que o particionamento NMR não reorganiza o padrão espacial do erro
de forma material.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    M1 monoescala Gassmann & 15,4 & 0,81 & $+0{,}59$ & $-0{,}17$ \\
    M3 multiescala NMR + Gassmann & 14,0 & 0,74 & $+0{,}30$ & $-0{,}13$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\VpVs{} concorda melhor com o sónico (erro médio absoluto $\approx 0{,}07$)
do que \Vp{} isolado (viés $+0{,}62$~km/s).
...
A fase~2b com Gassmann e $K_f$ default reduziu o viés de $+0{,}62$ para
$+0{,}59$~km/s (Tabela~\ref{tab:gassmann_compare}), confirmando que parte do
desvio seca/saturada é real---mas não suficiente para alinhar o perfil ao
DSI.
Etapa~3 deve focar resíduos locais (HFU~1, HFU~4) e não substituir a cadeia
física por ML direto.

\subsection{HFU~4 e HFU~3: zonas de cautela}

A HFU~4 não possui plug de calibração e apresenta MAPE $\approx 37\%$
...
($\approx 13\%$) é aceitável, mas não robusto.
A HFU~2 é o \emph{benchmark} interno do poço nesta etapa.
...
O modo \emph{forward} (medianas HFU, sem inversao no plug avaliado) replica
o MAPE \emph{in-sample} do M1 ($\approx 9\%$); o LOO melhora apenas
$\approx 2{,}5$~p.p., sem confirmacao \emph{in-sample}.
A fase~2g (Seção~\ref{sec:multiscale_nmr}) repete o exercicio com fracoes NMR
ponto a ponto no perfil de 87 linhas: o ganho MAPE frente ao M1 Gassmann e
de apenas $\approx 1{,}3$~p.p., abaixo do limiar de 2~p.p., e a validacao
CMR versus CT nos dez plugs mostra correlacao fraca ($r \approx -0{,}30$).
...
\begin{enumerate}
  \item \textbf{Viabilidade:} a modelagem DEM/SC calibrada por HFU é
        \emph{parcialmente consistente} com laboratório (LOO $\approx 15\%$
        nos dez plugs) e com sónico de perfil no poço~861 ($\approx 16\%$),
        com melhor desempenho em \VpVs{} do que em \Vp{} absoluto.
        Trata-se de \textbf{prova de conceito física local}, não de
        validação interpoços.
  \item \textbf{Uso por HFU:} confiar principalmente na HFU~2; usar HFU~1 e
        HFU~3 com ressalvas documentadas; evitar HFU~4 até obter plugs ou
  ...
  \item \textbf{Multiescala (fase~2f):} DEM sequencial meso-macro + micro
        ...
        ao perfil (MAPE \emph{in-sample} $\approx 9{,}1\%$ em ambos); manter
        $\alpha$ efetivo por HFU (Seção~\ref{sec:multiscale_ab}).
  \item \textbf{Multiescala NMR (fase~2g):} fracoes CMR ponto a ponto no
        perfil reduzem MAPE de 15,4\% para 14,0\% e viés de $+0{,}59$ para
        $+0{,}30$~km/s, mas o ganho ($\approx 1{,}3$~p.p.) fica abaixo do
        limiar de 2~p.p.; manter monoescala Gassmann
        (Seção~\ref{sec:multiscale_nmr}).
  \item \textbf{Fase~2b (Gassmann):} com $K_f \approx 2{,}2$~GPa e $S_w=1$, o
        viés contra o DSI cai de $+0{,}62$ para $+0{,}59$~km/s (MAPE 16,2\%
        $\rightarrow$ 15,4\%)---melhora parcial, não fechamento completo.
  \item \textbf{Etapa~3:} considerar aprendizado sobre resíduos da física
        (HFU~1 e HFU~4 prioritárias), não substituição da cadeia
...
\begin{table}[H]
  ...
  $\rho_m$ & $s_{K,G}$ \\
    \midrule
    1 & 4 & 0,554 & 0,950 & 84,0 & 36,9 & 58,6 & 25,8 & 2,774 & 0,698 \\
    2 & 4 & 0,554 & 0,191 & 83,2 & 36,3 & 58,7 & 25,6 & 2,766 & 0,706 \\
    3 & 2 & 0,615 & 0,950 & 81,8 & 35,3 & 59,0 & 25,5 & 2,754 & 0,721 \\
    4 & 0 & --    & 0,570 & 82,5 & 35,8 & 82,5 & 35,8 & 2,760 & 1,000 \\
    \bottomrule
  \end{tabular}
\end{table}
...
O sistema é resolvido numericamente com passo uniforme $\Delta y = 0{,}005$;
$(K_{\mathrm{dry}}, G_{\mathrm{dry}})$ correspondem ao ultimo ponto da
trajetoria em $y=\phi$.

Um teste de sensibilidade ao passo de integração, documentado na
Seção~\ref{sec:crosscheck_rockphypy} e na
Figura~\ref{fig:dem_rockphypy_crosscheck}, mostra variação inferior a 3\%
