...
% Compilar: pdflatex poco861_etapa1_beamer_DRP.tex
...
% Figuras originais + fundos extraidos do template DRP.
\graphicspath{%
  {./poco861_drp_assets/}%
...
% Fundo branco; o rodape DRP entra como footline real para reservar espaco.
\usebackgroundtemplate{}

% Titulo de frame no estilo DRP: preto, grande, sem barra superior.
...
% Rodape DRP como parte da area util do Beamer: evita sobreposicao do conteudo.
...
% Altura util abaixo do titulo e acima do rodape DRP (~1,12 cm).
\newlength{\drpbodyheight}
\setlength{\drpbodyheight}{0.62\textheight}

% #1 = fracao de drpbodyheight reservada so para a imagem (caption fica fora).
...
% Figura + legenda como bloco unico (evita corte da legenda).
...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    (campo Lapa)}\\[0.20cm]
    {\usebeamerfont{subtitle}\bfseries Integração de dados e modelos preditivos}\\[0.15cm]
    {\large Etapa~1 --- limites do perfil antes da física de rochas}\\[0.26cm]
    {\usebeamerfont{author} Romulo Brito da Silva}\\[0.08cm]
    {\usebeamerfont{institute} UENF / LENEP}\\[0.08cm]
    {\usebeamerfont{date} Junho de 2026}\\[0.22cm]
    {\footnotesize
    Intervalo 5205,91--5233,72~m $\cdot$ campo Lapa (pré-sal)\\
    87 amostras perfil+laboratório $\cdot$ 10 plugs com microtomografia (microCT)\\[0.10cm]
    Etapa diagnóstica: ainda não há produto de $V_p$/$V_s$}
  \end{center}
...
\begin{frame}{Roteiro}
  ...
  \begin{enumerate}
    ...
    \item Resultados do Random Forest: \philab, FZI, $k$ e HFU; comparação
...
\begin{frame}{O intervalo do poço~861 antes de modelar $V_p$}
  \footnotesize
  Carbonatos pré-sal do campo Lapa, $\sim$28~m (5206--5234~m).
  \philab, \klab{} e \fzilab{} variam bastante ($n=87$ de laboratório).
  \vspace{0.04cm}
  \begin{columns}[T,onlytextwidth]
    ...
    \slidefigure[0.7]{mogno_lab_properties_vs_depth}{Esquerda: \philab;
    centro: \klab{} (mD, escala log); direita: \fzilab{} vs.\ profundidade
    ($n=87$ pontos de laboratório).}
  \end{columns}
\end{frame}
...
\begin{frame}{Dois protocolos de validação}
  ...
  \vspace{-0.10cm}
  {\scriptsize
  Sem vazamento: lab não prediz lab.
  OOF = cada bloco (ou plug) é testado sem ter participado do treino.
  Evitamos sorteio 80/20, que infla $R^2$ neste tipo de dado.}

  \vspace{-0.22cm}
  \begin{block}{Falha no bloco~2 (Cenário~A)}
    \scriptsize Bloco~2 (5225--5234~m): faixa mais permeável e com mudança
    litológica; o RF de \philab{} treinado acima não generaliza ($R^2<0$).
  \end{block}
\end{frame}
...
\begin{frame}{Porosidade sim, FZI quase não}
  \begin{columns}[T,onlytextwidth]
    ...
    \slidefigure[0.9]{log_features_vs_FZI_lab_corr}{Correlação de Pearson:
    curvas de perfil versus \fzilab}
  \end{columns}
\end{frame}
...
\begin{frame}{Teste \emph{oracle}: FZI precisa de lab, não de perfil}
  \begin{columns}[T,onlytextwidth]
    ...
    algoritmo.}
    \column{0.48\textwidth}
    \slidefigure[0.9]{FZI_oracle_vs_logs_r2_by_fold}{$R^2$ por bloco: perfil
    isolado versus \emph{oracle}}
  \end{columns}
\end{frame}
...
\begin{frame}{RF nas 87 linhas: só \philab{} com $R^2$ OOF positivo}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    sinal wireline.\\[0.04cm]
    $R^2 \approx 0{,}15$ em \philab{} é modesto, mas positivo ---
    dá para usar como entrada de DEM/SC, sem substituir testemunho.}
    \column{0.46\textwidth}
    \slidefigure[0.9]{target_comparison_mean_r2}{\rSqOOF{} por alvo (RF);
...
\begin{frame}{\phiND{} direto supera o RF para \philab{}}
  \begin{columns}[T,onlytextwidth]
    ...
    {\scriptsize
    $r(\phiND{}, \philab) = 0{,}701$ ($n=87$).\\[0.03cm]
    \textbf{Acima:} perfil em profundidade.\\
    \textbf{Abaixo:} métricas e dispersão 1:1.}
    \column{0.67\textwidth}
    \centering
    \slidefig[0.65]{phi_nd_vs_rf_depth_profile}
  \end{columns}
  \vspace{0.04cm}
  \centering
  \slidefig[0.56]{phi_nd_vs_rf_bottom_panels}
\end{frame}
...
\begin{frame}{\philab{} RF: métricas OOF por bloco de profundidade}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.54\textwidth}
    \slidefigure[0.90]{phi_lab_oof_rmse_by_depth_fold}{RMSE OOF por bloco:
    RF, GB, MLP e LR (\philab{})}
  \end{columns}
\end{frame}
...
\begin{frame}{\philab{} em profundidade: predições OOF}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \slidefigure[0.72]{phi_lab_oof_depth_panels}{Observado (tracejado) versus
    OOF (colorido) por regressor}
...
\begin{frame}{MicroCT nos 10 plugs: RF não ganhou com CT}
  \begin{columns}[T,onlytextwidth]
    ...
    \end{itemize}
    \column{0.54\textwidth}
    \slidefigure[1.1]{plug_out_pred_vs_obs_rf}{FZI nos plugs: predito versus
    observado (LOPO, só perfil)}
  \end{columns}
\end{frame}
...
\begin{frame}{HFU pelo perfil: resultado fraco}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.52\textwidth}
    \slidefigure[0.9]{hfu_model_comparison_oof}{Métricas OOF: logística
    balanceada versus RF balanceado}
  \end{columns}
\end{frame}
...
\begin{frame}{GAM e Ridge não batem o RF em \philab}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.54\textwidth}
    \slidefigure[0.90]{phi_model_comparison_mean_r2}{$R^2$ médio: RF versus
    GAM e Ridge}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusões}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{enumerate}
      \item \phiND{} direto como porosidade para a física ($R^2 = 0{,}45$;
            RF OOF fica em 0,30).
      \item FZI só com perfil: não usar como produto.
  ...
  \end{columns}
  \vspace{0.12cm}
  \noindent\footnotesize O perfil ajuda na porosidade via \phiND{}; RF não
